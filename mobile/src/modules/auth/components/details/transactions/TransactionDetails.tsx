...
import {View, FlatList, ListRenderItem, StyleSheet, TouchableOpacity, Alert, Clipboard} from 'react-native';
...
export const TransactionDetails: React.FC<Props> = ({transaction}) => {
    ...
    const TouchableLineItem: ListRenderItem<TxKeyValue> = ({item}) => (
        <TouchableOpacity onPress={() => touchedItem(item.value)}>
            <LabeledTextField label={item.key} text={item.value} color={Colors.WHITE}/>
        </TouchableOpacity>
    );
        ...
        <View style={styles.container}>
            <View style={styles.header}>
                <TouchableOpacity onPress={() => touchedItem(transaction.transaction)}>
                    <Text color={Colors.WHITE} textAlign={TextAlign.CENTER}>{`Id: ${transaction.transaction}`}</Text>
                </TouchableOpacity>
                <Text size={FontSizes.SMALL} color={Colors.GREY} textAlign={TextAlign.CENTER}>{mountTxTypeString(transaction)}</Text>
            </View>
