...
export class HomeStackedAreaChart extends React.PureComponent<Props, State> {
    ...
    return (transactions, accountIndex) => {
      ...
      if (transactions && transactions.length) {
        for (let t = transactions.length - 1; t >= 0; t--) {
          ...
          if (
            showPricesIn === this.props.priceTypes[1] ||
            showPricesIn === this.props.priceTypes[2]
          ) {
            const priceOnThatDay = historicalPrice[atThatTime];
            data[i][`${ACCOUNT_TOKEN}${accountIndex}`] =
              transaction.balance * priceOnThatDay.close;
          } else {
  ...
  private buildChartData(
    ...
    for (
      ...
      if (
        this.props.priceApi.selectedCurrency === this.props.priceTypes[1] ||
  ...
  selectDateRange = () => {
    this.setState({
      ...this.state,
      selectedDateRange:
        DATE_RANGES[DATE_RANGES.indexOf(this.state.selectedDateRange) + 1] ||
        DATE_RANGES[0],
    });
  };
  ...
  render() {
    ...
    const getAccountColors = (colors: string[]): string[] => {
      return colors.length > this.props.accounts.length
        ? colors
        : getAccountColors(colors.concat(colors));
    };
