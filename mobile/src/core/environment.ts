...
import {BURSTALERTS_HOST_URL, CRYPTOCOMPARE_HOST_URL, DEFAULT_NODE_HOST, DEFAULT_PASSCODE_TIME, RELIABLE_NODE_HOSTS, BLACK_LISTED_ACCOUNT_IDS} from 'react-native-dotenv';

// So we check it like this
// tslint:disable-next-line: max-line-length
if (!DEFAULT_NODE_HOST ||
...
const fromCsvString = (csv: string): string[] => toString(csv).split(';').map(v => v.trim());
